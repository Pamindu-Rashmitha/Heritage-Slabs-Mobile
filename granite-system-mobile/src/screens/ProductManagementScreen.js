...
import {
    View, Text, StyleSheet, TouchableOpacity, FlatList, ActivityIndicator, Alert, StatusBar, Image,
} from 'react-native';
...
const StatsBar = ({ count }) => (
    <View style={styles.statsBar}>
        <Text style={styles.statsText}>{count} {count === 1 ? 'Product' : 'Products'} in Inventory</Text>
    </View>
);
...
const EmptyState = () => (
    <View style={styles.emptyContainer}>
        <MaterialCommunityIcons name="package-variant" size={72} color={THEME.textMuted} />
...
const ProductManagementScreen = ({ navigation }) => {
    ...
    const handleUpdate = (item) => { navigation.navigate('EditProduct', { product: item }); };

    const handleDelete = (item) => {
        Alert.alert('Delete Product', `Are you sure you want to remove "${item.stoneName}" from inventory?`, [
            ...
            {loading ? (
                ...
                <>
                    ...
                    <FlatList data={products} keyExtractor={(item) => item._id}
                        renderItem={({ item }) => <ProductRow item={item} onUpdate={handleUpdate} onDelete={handleDelete} />}
                        contentContainerStyle={[styles.listContent, products.length === 0 && styles.listContentEmpty]}
                        ListEmptyComponent={<EmptyState />} refreshing={refreshing} onRefresh={() => fetchProducts(true)} showsVerticalScrollIndicator={false} />
                </>
            )}
            <TouchableOpacity style={styles.fab} onPress={() => navigation.navigate('AddProduct')} activeOpacity={0.85}>
...
const styles = StyleSheet.create({
    ...
    row: { backgroundColor: THEME.bgCard, borderRadius: 14, marginBottom: 12, flexDirection: 'row', overflow: 'hidden', borderWidth: 1, borderColor: THEME.border },
    thumbContainer: { width: 72, height: 72, margin: 10, borderRadius: 10, overflow: 'hidden', alignSelf: 'center' },
    thumb: { width: '100%', height: '100%' },
    thumbPlaceholder: { width: '100%', height: '100%', backgroundColor: 'rgba(255,255,255,0.04)', justifyContent: 'center', alignItems: 'center', borderRadius: 10 },
    rowBody: { flex: 1, flexDirection: 'row', alignItems: 'center', paddingVertical: 14, paddingHorizontal: 14, gap: 10 },
    rowInfo: { flex: 1 },

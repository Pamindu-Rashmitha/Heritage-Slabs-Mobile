...
const PurchaseOrderManagementScreen = ({ navigation }) => {
    ...
    const fetchAllData = async (isRefresh = false) => { if (isRefresh) setRefreshing(true); else setLoading(true); try { const token = await AsyncStorage.getItem('userToken'); const h = { Authorization: `Bearer ${token}` }; const [poR, sR, pR] = await Promise.all([api.get('/purchase-orders', { headers: h }), api.get('/suppliers', { headers: h }), api.get('/products', { headers: h })]); setPurchaseOrders(poR.data.purchaseOrders ?? poR.data ?? []); setSuppliers(sR.data.suppliers ?? sR.data ?? []); setProducts(pR.data.products ?? pR.data ?? []); } catch (e) { Alert.alert('Fetch Error', 'Could not load data.'); } finally { setLoading(false); setRefreshing(false); } };
    const openAddForm = () => { setSelectedSupplierId(''); setSelectedProductId(''); setQuantity(''); setUnitCost(''); setExpectedDate(''); setFormModalVisible(true); };
    const validateForm = () => { if (!selectedSupplierId) { Alert.alert('Validation Error', 'Please select a supplier.'); return false; } if (!selectedProductId) { Alert.alert('Validation Error', 'Please select a product.'); return false; } if (!quantity.trim() || parseFloat(quantity) <= 0) { Alert.alert('Validation Error', 'Quantity must be a positive number.'); return false; } if (!unitCost.trim() || parseFloat(unitCost) <= 0) { Alert.alert('Validation Error', 'Unit cost must be a positive number.'); return false; } if (expectedDate.trim() && !/^\d{4}-\d{2}-\d{2}$/.test(expectedDate.trim())) { Alert.alert('Validation Error', 'Date must be in YYYY-MM-DD format.'); return false; } return true; };
    const handleSubmit = async () => { if (!validateForm()) return; setFormLoading(true); try { const token = await AsyncStorage.getItem('userToken'); const payload = { supplier: selectedSupplierId, product: selectedProductId, quantityInSqFt: parseFloat(quantity), unitCost: parseFloat(unitCost) }; if (expectedDate.trim()) payload.expectedArrivalDate = expectedDate.trim(); await api.post('/purchase-orders', payload, { headers: { Authorization: `Bearer ${token}` } }); Alert.alert('Success', 'Purchase order created.'); setFormModalVisible(false); fetchAllData(); } catch (e) { Alert.alert('Error', e.response?.data?.errors?.[0]?.msg || e.response?.data?.message || 'Could not create purchase order.'); } finally { setFormLoading(false); } };
    const confirmStatusUpdate = async (newStatus) => { setStatusModalVisible(false); try { const token = await AsyncStorage.getItem('userToken'); await api.put(`/purchase-orders/${selectedPO._id}`, { status: newStatus }, { headers: { Authorization: `Bearer ${token}` } }); if (newStatus === 'Arrived' && selectedPO.status !== 'Arrived') { Alert.alert('Stock Updated', `${selectedPO.product?.stoneName || 'Product'} stock increased by ${selectedPO.quantityInSqFt} SqFt.`); } fetchAllData(); } catch (e) { Alert.alert('Update Failed', 'Could not update status.'); } };
    const handleDelete = (item) => { const stockWarning = item.status === 'Arrived' ? '\n\nThis will also decrement the product stock.' : ''; Alert.alert('Delete Purchase Order', `Delete this purchase order?${stockWarning}`, [{ text: 'Cancel', style: 'cancel' }, { text: 'Delete', style: 'destructive', onPress: async () => { try { const token = await AsyncStorage.getItem('userToken'); await api.delete(`/purchase-orders/${item._id}`, { headers: { Authorization: `Bearer ${token}` } }); setPurchaseOrders(p => p.filter(po => po._id !== item._id)); } catch (e) { Alert.alert('Delete Failed', 'Could not delete this purchase order.'); } } }]); };
    ...
    return (
        <SafeAreaView style={s.container} edges={['top']}><StatusBar barStyle="light-content" backgroundColor={THEME.bg} />
            ...
            <Modal visible={formModalVisible} transparent animationType="slide"><View style={s.modalOverlay}><View style={[s.modalContent, { width: '90%' }]}>
                ...
                <TouchableOpacity style={s.input} onPress={() => setSupplierPickerVisible(true)}><Text style={{ color: selectedSupplierId ? THEME.textPrimary : THEME.textMuted, fontSize: 15 }}>{getSupplierLabel()}</Text></TouchableOpacity>
                <Text style={s.label}>Product</Text>
                <TouchableOpacity style={s.input} onPress={() => setProductPickerVisible(true)}><Text style={{ color: selectedProductId ? THEME.textPrimary : THEME.textMuted, fontSize: 15 }}>{getProductLabel()}</Text></TouchableOpacity>
                <Text style={s.label}>Quantity (SqFt)</Text>
                <TextInput style={s.input} placeholder="e.g. 500" placeholderTextColor={THEME.textMuted} value={quantity} onChangeText={setQuantity} keyboardType="numeric" />
                <Text style={s.label}>Unit Cost (LKR per SqFt)</Text>
                <TextInput style={s.input} placeholder="e.g. 150" placeholderTextColor={THEME.textMuted} value={unitCost} onChangeText={setUnitCost} keyboardType="numeric" />
                ...
                <TextInput style={s.input} placeholder="e.g. 2026-04-15" placeholderTextColor={THEME.textMuted} value={expectedDate} onChangeText={setExpectedDate} />

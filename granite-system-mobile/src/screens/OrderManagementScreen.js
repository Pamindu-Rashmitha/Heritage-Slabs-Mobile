...
const getStatusColor = (status) => {
    switch (status) {
        case 'Pending': return { text: COLORS.pending, bg: COLORS.pendingBg };
        case 'Processing': return { text: COLORS.processing, bg: COLORS.processingBg };
        case 'Shipped': return { text: COLORS.shipped, bg: COLORS.shippedBg };
        case 'Delivered': return { text: COLORS.delivered, bg: COLORS.deliveredBg };
        default: return { text: COLORS.textSub, bg: COLORS.bg };
    }
};
...
const OrderRow = ({ item, onUpdateStatus, onDelete }) => {
    ...
    const payColor = item.paymentStatus === 'Paid' ? { text: '#2a9d8f', bg: '#e8f5f4' } : item.paymentStatus === 'Failed' ? { text: '#e63946', bg: '#fdecea' } : { text: '#f4a261', bg: '#fdf3ea' };
            ...
            <View style={styles.rowBody}>
                <View style={styles.rowInfo}>
                    ...
                    <View style={styles.rowMeta}>
                        ...
                        <Text style={styles.metaText}>{productCount} item(s)</Text>
                    </View>
                    <View style={[styles.rowMeta, { marginBottom: 4 }]}>
                        {item.paymentMethod && <View style={[styles.badge, { backgroundColor: '#f3f4f6' }]}><Text style={[styles.badgeText, { color: COLORS.textSub }]}>{item.paymentMethod}{item.cardLastFour ? ` •••• ${item.cardLastFour}` : ''}</Text></View>}
...
const EmptyState = () => (
    <View style={styles.emptyContainer}>
        <MaterialCommunityIcons name="clipboard-list-outline" size={72} color={COLORS.border} />
...
const OrderManagementScreen = ({ navigation }) => {
    ...
    const fetchOrders = async (isRefresh = false) => {
        ...
        try {
            ...
            Alert.alert('Fetch Error', 'Could not load orders.');
                    ...
                    <View style={styles.modalContent}>
                        ...
                        {STATUS_FLOW.map((status) => (
                            <TouchableOpacity
                                ...
                                style={[
                                    styles.modalOption,
                                    selectedOrder?.status === status && styles.modalOptionActive,
                                ]}
                                onPress={() => confirmStatusUpdate(status)}
                            >
                                <Text style={[
                                    styles.modalOptionText,
                                    selectedOrder?.status === status && styles.modalOptionTextActive,
                                ]}>{status}</Text>
                            </TouchableOpacity>
                        ))}
                        <TouchableOpacity
...
const styles = StyleSheet.create({
    ...
    header: {
        backgroundColor: COLORS.dark, paddingHorizontal: 20, paddingTop: 16, paddingBottom: 24,
        flexDirection: 'row', alignItems: 'center', gap: 14,
        borderBottomLeftRadius: 24, borderBottomRightRadius: 24,
    },
    backBtn: { backgroundColor: 'rgba(255,255,255,0.12)', borderRadius: 12, padding: 9 },
    headerText: { flex: 1 },
    headerEyebrow: { fontSize: 11, fontWeight: '700', color: COLORS.accent, letterSpacing: 2, marginBottom: 2 },
    headerTitle: { fontSize: 22, fontWeight: '800', color: COLORS.white },
    statsBar: { flexDirection: 'row', alignItems: 'center', paddingHorizontal: 20, paddingVertical: 12 },
    ...
    row: {
        backgroundColor: COLORS.white, borderRadius: 14, marginBottom: 12, overflow: 'hidden',
        shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.07, shadowRadius: 6, elevation: 3,
    },
    rowBody: { flexDirection: 'row', alignItems: 'center', paddingVertical: 14, paddingHorizontal: 16, gap: 10 },
    rowInfo: { flex: 1 },
    rowName: { fontSize: 16, fontWeight: '700', color: COLORS.textPrimary, marginBottom: 6 },
    rowMeta: { flexDirection: 'row', alignItems: 'center', gap: 8, marginBottom: 4 },
    badge: { borderRadius: 6, paddingHorizontal: 8, paddingVertical: 3 },
    badgeText: { fontSize: 12, fontWeight: '600' },
    metaText: { fontSize: 12, color: COLORS.textSub },
    priceText: { fontSize: 13, fontWeight: '600', color: COLORS.textPrimary, marginBottom: 2 },
    addressText: { fontSize: 12, color: COLORS.textSub },
    ...
    updateBtn: { backgroundColor: COLORS.accentLight },
    ...
    modalContent: { backgroundColor: COLORS.white, borderRadius: 16, padding: 24, width: '80%' },
    modalTitle: { fontSize: 18, fontWeight: '700', color: COLORS.textPrimary, marginBottom: 16, textAlign: 'center' },
    modalOption: { paddingVertical: 12, paddingHorizontal: 16, borderRadius: 10, marginBottom: 8, backgroundColor: COLORS.bg },
    modalOptionActive: { backgroundColor: COLORS.accentLight, borderWidth: 1, borderColor: COLORS.accent },
    modalOptionText: { fontSize: 15, fontWeight: '500', color: COLORS.textPrimary, textAlign: 'center' },
    modalOptionTextActive: { color: COLORS.accent, fontWeight: '700' },
    modalCancel: { paddingVertical: 12, marginTop: 4 },
    modalCancelText: { fontSize: 15, fontWeight: '600', color: COLORS.textSub, textAlign: 'center' },
});

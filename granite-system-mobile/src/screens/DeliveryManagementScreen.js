...
const DeliveryRow = ({ item, onUpdateStatus, onDelete }) => {
    ...
    return (
        <View style={s.row}><View style={s.rowBody}><View style={s.rowInfo}>
            ...
            <View style={s.rowMeta}><View style={[s.badge,{backgroundColor:sc.bg}]}><Text style={[s.badgeText,{color:sc.text}]}>{item.status}</Text></View><Text style={s.metaText}>{item.vehicle?.licensePlate||'N/A'}</Text></View>
...
const DeliveryManagementScreen = ({ navigation }) => {
    const [deliveries, setDeliveries] = useState([]); const [orders, setOrders] = useState([]); const [vehicles, setVehicles] = useState([]);
    ...
    const [selectedOrderId, setSelectedOrderId] = useState(''); const [selectedVehicleId, setSelectedVehicleId] = useState('');
        ...
        <SafeAreaView style={s.container} edges={['top']}>
            ...
            {loading?(<View style={s.centered}><ActivityIndicator size="large" color={THEME.indigo}/><Text style={s.loadingText}>Loading Deliveries…</Text></View>):(
                ...
                <FlatList data={deliveries} keyExtractor={i=>i._id} renderItem={({item})=>(<DeliveryRow item={item} onUpdateStatus={i=>{setSelectedDelivery(i);setStatusModalVisible(true);}} onDelete={handleDelete}/>)}
                    ...
                    ListEmptyComponent={<View style={s.emptyContainer}><MaterialCommunityIcons name="truck-delivery-outline" size={72} color={THEME.textMuted}/><Text style={s.emptyTitle}>No Deliveries Yet</Text><Text style={s.emptySub}>Tap + to schedule a delivery.</Text></View>}
            ...
            <Modal visible={vehiclePickerVisible} transparent animationType="fade"><View style={s.modalOverlay}><View style={[s.modalContent,{width:'90%',maxHeight:'60%'}]}>
                ...
                <FlatList data={vehicles} keyExtractor={i=>i._id} renderItem={({item})=>(<TouchableOpacity style={[s.modalOption,selectedVehicleId===item._id&&s.modalOptionActive]} onPress={()=>{setSelectedVehicleId(item._id);setVehiclePickerVisible(false);}}><Text style={[s.modalOptionText,selectedVehicleId===item._id&&s.modalOptionTextActive]}>{item.licensePlate} ({item.vehicleType})</Text></TouchableOpacity>)} ListEmptyComponent={<Text style={{textAlign:'center',color:THEME.textSecondary,padding:20}}>No vehicles available</Text>}/>

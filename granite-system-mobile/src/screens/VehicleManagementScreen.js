...
const getStatusColor = (s) => { if(s==='Available') return {text:THEME.success,bg:THEME.successBg}; if(s==='On Route') return {text:THEME.warning,bg:THEME.warningBg}; if(s==='Maintenance') return {text:THEME.danger,bg:THEME.dangerBg}; return {text:THEME.textSecondary,bg:'rgba(255,255,255,0.06)'}; };

const VehicleRow = ({ item, onEdit, onDelete }) => {
    const sc = getStatusColor(item.status);
    return (<View style={st.row}><View style={st.rowBody}><View style={st.rowInfo}>
        <Text style={st.rowName} numberOfLines={1}>{item.licensePlate}</Text>
        <View style={st.rowMeta}><View style={[st.badge,{backgroundColor:sc.bg}]}><Text style={[st.badgeText,{color:sc.text}]}>{item.status}</Text></View><Text style={st.metaText}>{item.vehicleType}</Text></View>
...
const VehicleManagementScreen = ({ navigation }) => {
    const [vehicles, setVehicles] = useState([]); const [loading, setLoading] = useState(true); const [refreshing, setRefreshing] = useState(false);
    ...
    const fetchVehicles = async (isRefresh=false) => { if(isRefresh) setRefreshing(true); else setLoading(true); try { const token = await AsyncStorage.getItem('userToken'); const res = await api.get('/vehicles',{headers:{Authorization:`Bearer ${token}`}}); setVehicles(res.data.vehicles??res.data??[]); } catch(e){ Alert.alert('Fetch Error','Could not load vehicles.'); } finally { setLoading(false); setRefreshing(false); } };
    ...
    const validateForm = () => { if(!licensePlate.trim()){Alert.alert('Validation Error','License plate is required.');return false;} if(licensePlate.trim().length<2){Alert.alert('Validation Error','License plate must be at least 2 characters.');return false;} if(!vehicleType.trim()){Alert.alert('Validation Error','Vehicle type is required.');return false;} if(vehicleType.trim().length<2){Alert.alert('Validation Error','Vehicle type must be at least 2 characters.');return false;} if(!capacity.trim()){Alert.alert('Validation Error','Max weight capacity is required.');return false;} const cap = parseFloat(capacity); if(isNaN(cap)||cap<=0){Alert.alert('Validation Error','Capacity must be a valid positive number.');return false;} return true; };
    const handleSubmit = async () => { if(!validateForm()) return; setFormLoading(true); try { const token = await AsyncStorage.getItem('userToken'); const body = { licensePlate:licensePlate.trim(), vehicleType:vehicleType.trim(), maxWeightCapacity:parseFloat(capacity), status }; if(editing) { await api.put(`/vehicles/${editing._id}`,body,{headers:{Authorization:`Bearer ${token}`}}); Alert.alert('Success','Vehicle updated successfully.'); } else { await api.post('/vehicles',body,{headers:{Authorization:`Bearer ${token}`}}); Alert.alert('Success','Vehicle added successfully.'); } setFormVisible(false); fetchVehicles(); } catch(e){ Alert.alert('Error',e.response?.data?.message||e.response?.data?.errors?.[0]?.msg||'Could not save vehicle.'); } finally { setFormLoading(false); } };
    const handleDelete = (item) => { Alert.alert('Delete Vehicle',`Delete "${item.licensePlate}"?`,[{text:'Cancel',style:'cancel'},{text:'Delete',style:'destructive',onPress:async()=>{ try { const token = await AsyncStorage.getItem('userToken'); await api.delete(`/vehicles/${item._id}`,{headers:{Authorization:`Bearer ${token}`}}); setVehicles(p=>p.filter(v=>v._id!==item._id)); } catch(e){ Alert.alert('Delete Failed','Could not delete vehicle.'); } }}]); };
        ...
        <SafeAreaView style={st.container} edges={['top']}><StatusBar barStyle="light-content" backgroundColor={THEME.bg}/>
            ...
            {loading?(<View style={st.centered}><ActivityIndicator size="large" color={THEME.purple}/><Text style={st.loadingText}>Loading Vehicles…</Text></View>):(
                ...
                <FlatList data={vehicles} keyExtractor={i=>i._id} renderItem={({item})=>(<VehicleRow item={item} onEdit={openEdit} onDelete={handleDelete}/>)}
                    ...
                    ListEmptyComponent={<View style={st.emptyContainer}><MaterialCommunityIcons name="car-outline" size={72} color={THEME.textMuted}/><Text style={st.emptyTitle}>No Vehicles Yet</Text><Text style={st.emptySub}>Tap + to add a vehicle.</Text></View>}
            ...
            <Modal visible={formVisible} transparent animationType="slide"><View style={st.modalOverlay}><View style={[st.modalContent,{width:'90%'}]}>
                ...
                <Text style={st.label}>License Plate</Text><TextInput style={st.input} placeholder="e.g. AB-1234" placeholderTextColor={THEME.textMuted} value={licensePlate} onChangeText={setLicensePlate}/>
                ...
                <Text style={st.label}>Max Weight Capacity (kg)</Text><TextInput style={st.input} placeholder="e.g. 5000" placeholderTextColor={THEME.textMuted} value={capacity} onChangeText={setCapacity} keyboardType="numeric"/>
            ...
            <Modal visible={statusPickerVisible} transparent animationType="fade"><View style={st.modalOverlay}><View style={st.modalContent}>
                ...
                {STATUS_OPTIONS.map(s=>(<TouchableOpacity key={s} style={[st.modalOption,status===s&&st.modalOptionActive]} onPress={()=>{setStatus(s);setStatusPickerVisible(false);}}><Text style={[st.modalOptionText,status===s&&st.modalOptionTextActive]}>{s}</Text></TouchableOpacity>))}

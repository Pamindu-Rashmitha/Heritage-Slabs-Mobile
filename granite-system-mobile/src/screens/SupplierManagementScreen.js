...
const SupplierRow = ({ item, onEdit, onDelete }) => (
    ...
    <View style={st.rowActions}>
        <TouchableOpacity style={[st.actionBtn,st.updateBtn]} onPress={()=>onEdit(item)} activeOpacity={0.8}><MaterialCommunityIcons name="pencil-outline" size={18} color={THEME.indigo}/><Text style={[st.actionText,{color:THEME.indigo}]}>Edit</Text></TouchableOpacity>
...
const SupplierManagementScreen = ({ navigation }) => {
    ...
    return (
        <SafeAreaView style={st.container} edges={['top']}><StatusBar barStyle="light-content" backgroundColor={THEME.bg}/>
            ...
            {loading?(<View style={st.centered}><ActivityIndicator size="large" color={THEME.indigo}/><Text style={st.loadingText}>Loading Suppliers…</Text></View>):(
...
const st = StyleSheet.create({
    ...
    backBtn:{backgroundColor:'rgba(255,255,255,0.08)',borderRadius:12,padding:9},headerText:{flex:1}, headerEyebrow:{fontSize:11,fontWeight:'700',color:THEME.indigo,letterSpacing:2,marginBottom:2},headerTitle:{fontSize:22,fontWeight:'800',color:THEME.textPrimary},

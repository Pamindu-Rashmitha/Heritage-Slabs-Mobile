    ...
    return (
        <TouchableOpacity style={st.row} onPress={() => onOpen(item)} activeOpacity={0.85}>
            <View style={st.rowBody}><View style={st.rowInfo}>
                ...
                <View style={st.rowMeta}><View style={[st.badge,{backgroundColor:sc.bg}]}><Text style={[st.badgeText,{color:sc.text}]}>{item.status}</Text></View><View style={[st.badge,{backgroundColor:tc.bg}]}><Text style={[st.badgeText,{color:tc.text}]}>{item.type}</Text></View></View>
...
const TicketManagementScreen = ({ navigation }) => {
    ...
    const [detailModalVisible, setDetailModalVisible] = useState(false); const [selectedTicket, setSelectedTicket] = useState(null);
        ...
        <SafeAreaView style={st.container} edges={['top']}><StatusBar barStyle="light-content" backgroundColor={THEME.bg} />
            ...
            {loading ? (<View style={st.centered}><ActivityIndicator size="large" color={THEME.warning} /><Text style={st.loadingText}>Loading Tickets…</Text></View>) : (
                ...
                <FlatList data={tickets} keyExtractor={i => i._id} renderItem={({ item }) => (<TicketRow item={item} onOpen={openDetail} onDelete={handleDelete} />)}
                    ...
                    ListEmptyComponent={<View style={st.emptyContainer}><MaterialCommunityIcons name="face-agent" size={72} color={THEME.textMuted} /><Text style={st.emptyTitle}>No Tickets Yet</Text><Text style={st.emptySub}>Customer tickets will appear here.</Text></View>}
                    refreshing={refreshing} onRefresh={() => fetchTickets(true)} showsVerticalScrollIndicator={false} /></>
            )}
            <Modal visible={detailModalVisible} transparent animationType="fade"><View style={st.modalOverlay}><View style={st.detailModalContent}><ScrollView showsVerticalScrollIndicator={false}>
                <Text style={st.modalTitle}>Ticket Details</Text>
                <Text style={st.detailLabel}>Subject</Text><Text style={st.detailValue}>{selectedTicket?.subject}</Text>
                <Text style={st.detailLabel}>Customer</Text><Text style={st.detailValue}>{selectedTicket?.user?.name || 'Unknown'} ({selectedTicket?.user?.email || '-'})</Text>
                <Text style={st.detailLabel}>Order</Text><Text style={st.detailValue}>#{selectedTicket?.order?._id?.slice(-6).toUpperCase() || 'N/A'}{selectedTicket?.order?.totalPrice ? ` — LKR ${selectedTicket.order.totalPrice.toLocaleString()}` : ''}</Text>
                <Text style={st.detailLabel}>Type</Text><View style={[st.badge, { backgroundColor: getTypeColor(selectedTicket?.type).bg, alignSelf: 'flex-start' }]}><Text style={[st.badgeText, { color: getTypeColor(selectedTicket?.type).text }]}>{selectedTicket?.type}</Text></View>
                <Text style={st.detailLabel}>Description</Text><Text style={st.detailValue}>{selectedTicket?.description}</Text>
                <Text style={st.detailLabel}>Status</Text>
                <View style={st.statusRow}>{STATUS_FLOW.map(s => (<TouchableOpacity key={s} style={[st.statusChip, newStatus === s && st.statusChipActive]} onPress={() => setNewStatus(s)}><Text style={[st.statusChipText, newStatus === s && st.statusChipTextActive]}>{s}</Text></TouchableOpacity>))}</View>
                <Text style={st.detailLabel}>Admin Reply</Text>
                <TextInput style={st.replyInput} placeholder="Type your reply to the customer..." placeholderTextColor={THEME.textMuted} value={replyText} onChangeText={setReplyText} multiline />
                <TouchableOpacity style={[st.saveBtn, updating && { opacity: 0.6 }]} onPress={handleUpdate} disabled={updating} activeOpacity={0.85}>{updating ? <ActivityIndicator color="#fff" /> : <Text style={st.saveBtnText}>Save Changes</Text>}</TouchableOpacity>
...
const st = StyleSheet.create({
    ...
    descText: { fontSize: 12, color: THEME.textSecondary, fontStyle: 'italic' },
    ...
    modalTitle: { fontSize: 20, fontWeight: '800', color: THEME.textPrimary, marginBottom: 16, textAlign: 'center' },
    detailLabel: { fontSize: 11, fontWeight: '700', color: THEME.textSecondary, letterSpacing: 1, marginTop: 14, marginBottom: 4, textTransform: 'uppercase' },
